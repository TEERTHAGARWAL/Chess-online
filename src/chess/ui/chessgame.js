...
const ChessGameWrapper = (props) => {
	...
	const [ resign, setResign ] = React.useState(false);
	const [ oppdraw, setOppDraw ] = React.useState(false);
	const [ youResign, setYouResign ] = React.useState(false);
	const [ middleState, setMiddleState ] = React.useState(false);
	const [ oppresign, setOppResign ] = React.useState(false);
	...
	React.useEffect(() => {
		...
		//oppoenent draw request
		socket.on('draw request', (data) => {
			if (data.socketid !== socket.id) setOppDraw(true);
		});

		// opponenet has resigned
		socket.on('regined', (data) => {});
		...
		socket.on('Opponent Accepted Draw', (data) => {
			if (data.socketid !== socket.id) setMiddleState(true);
		});
		...
		socket.on('resign request', (data) => {
			if (socket.id !== data.socketid) setOppResign(true);
		});
	}, []);
	const redirectHome = () => {
		history.push('/');
		...
		if (oppresign) {
			return (
				<Alert className="alert alert-success" role="alert">
					<h2>Opponent Has Resigned</h2>
					<h2>Winner: {props.myUserName}</h2>
					<div>
						<button
							className="btn btn-success"
							onClick={() => {
				...
				<Alert className="alert alert-warning" role="alert">
					...
					<div>
						<button
							...
							onClick={() => {
								if (draw) socket.emit('draw request', gameid);
								if (resign) {
									socket.emit('resign request', gameid);
									setYouResign(true);
								}
	...
	const matchdraw = () => {
		if (middleState) {
			socket.emit('Opponent Accepted Draw', gameid);
			return (
				<Alert className="alert alert-success" role="alert">
					<h2>Match Tied</h2>
					<div>
	...
	const OpponentDrawRequest = () => {
		if (oppdraw) {
			return (
				<Alert className="alert alert-warning" role="alert">
					<h2>Opponent Has Asked for a Draw</h2>
					<div>
						<button
